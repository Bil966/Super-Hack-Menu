...
namespace superhack::hacks::Player {
    class $hack(Noclip) {
        void init() override {
    ...
    class $modify(NoClipPLHook, PlayLayer) {
        ...
        bool shouldDestroyByLimit() {
            if (config::get<bool>("player.noclip.acclimit.toggle", false)) {
                double acc = config::getTemp<double>("noclipAccuracy", 100.0);
                double limit = config::get<double>("player.noclip.acclimit", 95.0);
                if (acc < limit) return true;
            }
        ...
        void postUpdate(float dt) override {
            ...
            if (config::get<bool>("player.noclip.tint", false) && fields->m_noclipTint && !m_hasCompletedLevel && !m_player1->m_isDead) {
                ...
                if (time == 0.f) {
                    // fade in/out smoothly using dt
                    float speed = dt * 5.f;
                    if (fields->m_wouldDie) {
                        fields->m_tintOpacity = std::min(1.f, fields->m_tintOpacity + speed);
                        fields->m_wouldDie = false;
                    } else {
                        fields->m_tintOpacity = std::max(0.f, fields->m_tintOpacity - speed);
                    }
                    fields->m_noclipTint->setOpacity(fields->m_tintOpacity * config::get<double>("player.noclip.opacity", 90.f));
                } else {
                    if (fields->m_wouldDie) {
                        ...
                        if (progress >= 1.0f) {
                            ...
                            fields->m_noclipTint->setOpacity(fields->m_tintOpacity);
                        }
                    }
                }
            }
        ...
        void processNoclipDeaths() {
            ...
            if (fields->m_wouldDieFrame) {
                ...
                if (!fields->m_deadLastFrame) {
                    ...
                    if (config::get<bool>("player.noclip.deathlimit.toggle", false) && deaths + 1 >= config::