...
namespace superhack::Player {
    ...
    // Chuyển tọa độ màn hình sang tọa độ trong game
    static cocos2d::CCPoint screenToGame(cocos2d::CCPoint const& screenPos, PlayLayer* playLayer) {
        ...
        return ccp(cameraPos.x + scaledPos.x, cameraPos.y + scaledPos.y);
    }

    // Chuyển tọa độ chuột sang frame game
    ...
    class $hack(ClickTeleport) {
        ...
        void update() override {
            if (!config::get<"player.clicktp", bool>(false)) return;
            // Luôn hiển thị con trỏ khi hack bật
