...
namespace superhack::Player {
    ...
    // Custom action để xử lý spin thêm vòng
    ...
    public:
        ...
        void step(float dt) override {
            ...
            if (m_innerAction->isDone()) {
                ...
                if (!m_madeExtraTurn) {
                    // restart action để thêm vòng quay
    ...
    class $modify(BallRotationPOHook, PlayerObject) {
        ...
        struct Fields {
            ...
            Fields() {
                ...
                m_spinNode->onEnter(); // enable scheduler cho dummy node
            }
        };
        ...
        void switchedToMode(GameObjectType p0) {
            ...
            if (p0 == GameObjectType::CubePortal || p0 == GameObjectType::RobotPortal) return;
        ...
        void prepareForRotation(float mult) {
            if (!config::get<bool>("player.ballrotation.variation", false)) return;
