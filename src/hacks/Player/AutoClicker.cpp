...
namespace superhack::Player {
    class $hack(AutoClicker) {
        void init() override {
            ...
            config::setIfEmpty<int>("player.autoclick.intervalhold", 1);
        ...
        [[nodiscard]] bool isCheating() const override { 
            return config::get<"player.autoclick", bool>(); 
        }
    ...
    class $modify(AutoClickerBGLHook, GJBaseGameLayer) {
        ...
        void updateAutoClicker() {
            ...
            fields->timer++;
            bool toggleClick = (!fields->clicking && fields->timer >= clickInterval) ||
                               (fields->clicking && fields->timer >= releaseInterval);

            if (toggleClick) {
                fields->clicking = !fields->clicking;
                if (config::get<"player.autoclick.p1", bool>(false)) 
                    this->handleButton(fields->clicking, 1, true);
                if (config::get<"player.autoclick.p2", bool>(false)) 
                    this->handleButton(fields->clicking, 1, false);
                fields->timer = 0;
            }
        }
