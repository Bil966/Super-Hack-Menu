...
namespace superhack::Player {
    ...
    class $modify(WaveTrailSizeHSHook, HardStreak) {
        ...
        void updateStroke(float dt) {
            ...
            // Gán streak cho player1 và player2
            if (s_currentStreak == nullptr || s_currentStreak2 == nullptr) {
                if (auto PL = utils::get<PlayLayer>()) {
                    if (PL->m_player1 && s_currentStreak == nullptr && this == PL->m_player1->m_waveTrail) {
                        s_currentStreak = this;
                        PL->m_player1->m_waveTrail->updateStroke(dt);
                    }
                    if (PL->m_player2 && s_currentStreak2 == nullptr && this == PL->m_player2->m_waveTrail) {
    ...
    class $modify(WaveTrailStrokeHSHook, cocos2d::CCDrawNode) {
        ...
        bool drawPolygon(cocos2d::CCPoint *verts, unsigned int count, const cocos2d::ccColor4F &fillColor, float borderWidth, const cocos2d::ccColor4F &borderColor, cocos2d::BorderAlignment alignment) {
            ...
            if (glowLayers == 0) {
                ...
                for (unsigned int i = 0; i < count; i++) newVerts[i] = verts[i];
                float offset = width + (width / count);
                newVerts[0].y -= offset; newVerts[3].y -= offset;
                newVerts[1].y += offset; newVerts[2].y += offset;
            ...
            for (int i = 0; i < glowLayers; i++) {
                ...
                std::vector<CCPoint> newVerts(count);
                for (unsigned int j = 0; j < count; j++) newVerts[j] = verts[j];
